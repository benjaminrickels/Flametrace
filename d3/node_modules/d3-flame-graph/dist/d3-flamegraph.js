...
/* harmony default export */ function flamegraph() {
    ...
    // whole color space palette
    const colors = [[0, 186, 255], [11, 95, 217], [0, 20, 240], [62, 11, 217], [150, 0, 255],
    ...
    // hash a threadid to a index into the color palette
    // same tid -> same index
    let hashThreadtoIndex = function (d) {
      return Math.abs(hashCode((typeof d.data.thread_uid !== 'undefined') ?
        (String(d.data.thread_uid) + " # " + String(d.data.thread_uid)) : ignoreName) % colors.length);
    }

    let colorMapper = function (d) {
        // return d.highlight ? '#E600E6' : colorHash(getName(d), getLibtype(d))
      var colorindex = hashThreadtoIndex(d);
      return `rgb(${colors[colorindex][0]},${colors[colorindex][1]},${colors[colorindex][2]})`;
    ...
    function update () {
        selection.each(function (root) {
            ...
            g.attr('width', width)
                ...
                // LH hide ignored slices
                // Todo just remove them when building the DOM?
                .style('display', function (d) { return ((d.data.name) === ignoreName) ? 'none' : '' })
              
            // LH or just remove them outright
            // nvm this is slow as hell
            // var r = document.getElementsByName(ignoreName)
